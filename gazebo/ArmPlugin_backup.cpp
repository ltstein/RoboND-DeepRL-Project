...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( newReward && agent != NULL )
	{
		...
		if( endEpisode )
		{
			...
			totalRuns++;
			printf("Current Accuracy:  %0.4f (%03u of %03u) Base Grab Accuracy: %0.4f  (reward=%+0.2f %s)\n", float(successfulGrabs)/float(totalRuns), successfulGrabs, totalRuns, float(successfulGrabsBase)/float(totalRuns), rewardHistory, (rewardHistory >= REWARD_WIN ? "WIN" : "LOSS"));
			char checkpointFile[80];
			sprintf(checkpointFile, "Checkpoints/%d", totalRuns);
			agent->SaveCheckpoint(checkpointFile);
			printf("Agent checkpoint saved to %s", checkpointFile);
